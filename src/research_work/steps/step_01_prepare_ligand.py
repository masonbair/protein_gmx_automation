...
Prerequisites (user has already done manually):
  - REC.pdb, LIG.pdb, LIG.itp, all .mdp files, and the force field
    directory are present in config.SIM_DIR.
"""
...
def verify_inputs():
    """Check that all required files exist in config.SIM_DIR before starting."""
    required = [RECEPTOR_PDB, "LIG.pdb", "LIG.itp"]
    missing = [f for f in required if not (config.SIM_DIR / f).exists()]
    if missing:
        logger.error("Missing files in %s:", config.SIM_DIR)
        for f in missing:
            logger.error("  - %s", f)
...
def convert_ligand_to_gro():
    run_gmx("editconf", ["-f", "LIG.pdb", "-o", "LIG.gro"], work_dir=config.SIM_DIR)
    console.produced("LIG.gro")


def merge_ligand_into_conf():
    merge_gro_files(config.SIM_DIR / "conf.gro", config.SIM_DIR / "LIG.gro")
